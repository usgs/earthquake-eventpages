...
const WORLD_BOUNDS = [[85.0, 179.0], [-85.0, -179.0]];
...
export class MapComponent extends AbstractForm
  ...
  get location(): Location {
    return this.feltReport.location;
  }
  ...
  updatePin(feltReport = this.feltReport, event = this.event) {
    ...
    if (latitude === 0 && longitude === 0) {
      ...
      !(
        this.pin &&
        this.pin._map &&
        this.pin._map.getBounds().contains(latLng) &&
        this.pin._map.getZoom() > 5
      )
    ) {
      // Only update bounds if current map extent either (a) does not
      // contain the new location, or (b) is really zoomed out
      this.mapBounds = [
