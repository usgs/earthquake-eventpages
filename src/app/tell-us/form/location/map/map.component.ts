...
export class MapComponent extends AbstractForm implements OnChanges, OnInit {
  ...
  updateFeltReportLocation(feltReport = this.feltReport, event = this.event) {
    console.log('map.updateAddress');

    const latLng = this.getLatLng();
    if (latLng) {
      const coordinates = event.geometry.coordinates;
      this.feltReport.ciim_mapAddress = latLng[0] + ', ' + latLng[1];
      this.feltReport.ciim_mapLat = latLng[0];
      this.feltReport.ciim_mapLon = latLng[1];
    }
