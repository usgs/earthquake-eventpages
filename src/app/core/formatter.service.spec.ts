...
describe('FormatterService', () => {
  ...
  describe('date', () => {
    ...
    it('formats date as YYYY-MM-DD', inject(
      ...
      (formatter: FormatterService) => {
        ...
        expect(formatter.date(new Date(testDate.setUTCMonth(10)))).toEqual(
          '2001-11-02'
        );
        expect(formatter.date(new Date(testDate.setUTCDate(10)))).toEqual(
          '2001-11-10'
        );
      }
    ));
  ...
  describe('leftpad', () => {
    it('does not pad longer strings', inject(
      [FormatterService],
      (formatter: FormatterService) => {
        expect(formatter.leftPad('longer string', 2, 'X')).toEqual(
          'longer string'
        );
      }
    ));
    ...
    it('pads out a shorter string', inject(
      [FormatterService],
      (formatter: FormatterService) => {
        expect(formatter.leftPad('short', 10, '-')).toEqual('-----short');
      }
    ));
  });
    ...
    it('formats as expected', inject(
      ...
      (formatter: FormatterService) => {
        expect(formatter.location(1.23, 2.34)).toEqual(
          formatter.latitude(1.23) + ' ' + formatter.longitude(2.34)
        );

        expect(formatter.location(1.23, 2.34, 6)).toEqual(
          formatter.latitude(1.23, 6) + ' ' + formatter.longitude(2.34, 6)
        );
      }
    ));
  });
  ...
  describe('number', () => {
    ...
    it('returns empty value when value is "Random String"', inject(
      [FormatterService],
      (formatter: FormatterService) => {
        expect(formatter.number('Random String', 1, 'empty')).toEqual('empty');
      }
    ));
  ...
  describe('time', () => {
    ...
    it('formats time', inject(
      [FormatterService],
      (formatter: FormatterService) => {
        expect(formatter.time(testDate)).toEqual('01:02:03');
    ...
    it('includes milliseconds when requested', inject(
      [FormatterService],
      (formatter: FormatterService) => {
        expect(formatter.time(testDate, true)).toEqual('01:02:03.123');
        expect(formatter.time(new Date(testDate.getTime() - 30), true)).toEqual(
          '01:02:03.093'
        );
        expect(
          formatter.time(new Date(testDate.getTime() - 120), true)
        ).toEqual('01:02:03.003');
      }
    ));
  });
  ...
  describe('uncertainty', () => {
    ...
    it('uses decimals to round', inject(
      [FormatterService],
      (formatter: FormatterService) => {
        expect(formatter.uncertainty(1.2366, 3)).toEqual('&plusmn; 1.237');
      }
    ));
